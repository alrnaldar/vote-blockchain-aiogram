...
class SimpleBlockchain:
    def __init__(self):
        ...
        result_row = result[0]
        index, previous_hash, timestamp, data, hash_value = result_row

        self.prev_block = Block(index, previous_hash, timestamp, data, hash_value)
    ...
    def add_block(self, data):
        ...
        DB.cursor.execute(f"INSERT INTO blockchain (index, prev_hash, timestamp, data, hash) VALUES ('{new_block.index}', '{new_block.previous_hash}', to_timestamp({new_block.timestamp}), '{new_block.data}', '{new_block.hash}')")
        DB.conn.commit()
        return new_block
